...
In case of an invalid manager, I have used `react-toastify` to display the error in a banner form (to test, simply add an invalid user in `data.tsx`), while displaying the organisation heirarchy, where as for an employee with no manager id, he becomes a CEO as specified.
